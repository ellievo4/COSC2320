	...
	if (schd=="roundrobin")
	{
		...
		while (getline(fin,line))
		{
			...
			if (pid==1)
			{
				if(dura<=qt) 
				{
					q1.enqueue(arTi,dura,pid);
					time = dura;
				}
				else
				{
					q1.enqueue(arTi,qt,pid);
					q2.enqueue(0,dura-qt,pid);
					time = qt;
				}
				pid++;
			}
			...
			else
			{
				...
				{
					if(dura<=qt) 
						q1.enqueue(q1.getAT(),dura,pid);
					else
					{
						q1.enqueue(q1.getAT(),qt,pid);
						q2.enqueue(0,dura-qt,pid);
					}	
					pid++;
				}
				...
				else if (arTi>time)
				{
					...
					{
						q1.getDataRR(tempID, tempDur, tempAt);
						end_time = q1.getATFront();
						end_queue_process = q1.getAT();
						cout << setw(4) << tempAt <<  "\t" << setw(9) << tempID << "\t" << setw(8) << tempDur << "\t" << "start" << endl;
						fout << setw(4) << tempAt <<  "\t" << setw(9) << tempID << "\t" << setw(8) << tempDur << "\t" << "start" << endl;
						...
						if (tempDur<=qt)
							q1.enqueue(end_queue_process,tempDur,tempID);
						else
						{
							q1.enqueue(end_queue_process,qt,tempID);
							q2.enqueue(0,tempDur-qt,tempID);
						}
						q2.dequeue();
					}
